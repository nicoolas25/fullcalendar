...
;;
// This file is a duplicate of the Agenda view.
// It is patched for resource display.
...
function ResourceEventRenderer() {
  ...
  function compileSlotSegs(events) {
    ...
    for (i=0; i<colCnt; i++) {
      // The day is always the startDate.
      // cellDate = cellToDate(0, i);
  ...
  // PATCHED
  function sliceSegs(events, rangeStart, rangeEnd, resourceIndex) {
    ...
    for (i=0; i<len; i++) {
      ...
      // Only do the following for the given resourceIndex
      if (window.resourceList[resourceIndex].id === event.resourceId) {
...
function ResourceView(element, calendar, viewName) {
  ...
  function buildDayTableHeadHTML() {
    ...
    for (col=0; col<colCnt; col++) {
      // Even if ther is multiple columns (one for each resource),
      // the date is still considered the same.
      //date = cellToDate(0, col);
      date = cellToDate(0, 0);
      html +=
        "<th class='fc-" + dayIDs[date.day()] + " fc-col" + col + ' ' + headerClass + "'>" +
        ...
        //htmlEscape(formatDate(date, colFormat)) +
        htmlEscape(window.resourceList[col].name) +
        "</th>";
    }
